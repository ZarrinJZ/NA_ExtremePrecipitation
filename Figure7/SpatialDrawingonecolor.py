...
def plot(X, Y, values, lons, lats, key):
    ...
    if zhishu == "cdd":
        ...
        p = ax.contourf(X, Y, values, cmap=trunc_cmap, levels=np.linspace(-5, 45, 11), extend='both')
    # 设置colorbar
    # plt.colorbar(p, format='%.1f', orientation="horizontal",  extend='both', aspect=29, fraction=0.03, pad=0.09)
    ...
    plt.scatter(lons, lats, color="black", s=1.5)
    ...
    # plt.tight_layout()
    plt.savefig("chapter4.3_"+titleout + '.png', format='png', transparent=True, dpi=100, pad_inches=0.0)
    plt.show()
