...
def plot(X, Y, values, lons, lats, key):
    ...
    if zhishu == "cdd":
        p = plt.pcolormesh(X, Y, values, cmap="RdBu_r", vmin=0, vmax=60)
    elif zhishu == "cwd":
        p = plt.pcolormesh(X, Y, values, cmap="RdBu_r", vmin=0, vmax=20)
    elif zhishu == "prcptot":
        p = plt.pcolormesh(X, Y, values, cmap="RdBu_r", vmin=0, vmax=1500)
    elif zhishu == "sdii":
        p = plt.pcolormesh(X, Y, values, cmap="RdBu_r", vmin=0, vmax=15)
    elif zhishu == "r10mm":
        p = plt.pcolormesh(X, Y, values, cmap="RdBu_r", vmin=0, vmax=40)
    elif zhishu == "r20mm":
        p = plt.pcolormesh(X, Y, values, cmap="RdBu_r", vmin=0, vmax=20)
    elif zhishu == "r95p":
        p = plt.pcolormesh(X, Y, values, cmap="RdBu_r", vmin=0, vmax=400)
    elif zhishu == "r99p":
        p = plt.pcolormesh(X, Y, values, cmap="RdBu_r", vmin=0, vmax=100)
    elif zhishu == "r95ptot":
        p = plt.pcolormesh(X, Y, values, cmap="RdBu_r", vmin=10, vmax=30)
    elif zhishu == "r99ptot":
        p = plt.pcolormesh(X, Y, values, cmap="RdBu_r", vmin=0, vmax=10)
    elif zhishu == "rx1day":
        p = plt.pcolormesh(X, Y, values, cmap="RdBu_r", vmin=0, vmax=60)
    elif zhishu == "rx5day":
        p = plt.pcolormesh(X, Y, values, cmap="RdBu_r", vmin=0, vmax=120)
    # 设置colorbar
    ...
    region_overlay = gpd.read_file("/Volumes/OneTouch/CMIP6_Region_NA_SUB/NorthAmerica/NorthAmerica.shp")
    region_overlay.plot(ax=ax, color="none", edgecolor='black', zorder=100, alpha=0.8, linewidth=2)
    plt.tight_layout()
    plt.savefig("chapter4.3_"+titleout + '.png', format='png', transparent=True, dpi=100, pad_inches=0.1, bbox_inches='tight')
    # plt.show()
...
if __name__ == '__main__':
    rootpath = "/Volumes/OneTouch/Calculated_Indices_FutureAnalysis/1_RMSE_Evaluation/SNR_Calculation/spatial_SNR"
